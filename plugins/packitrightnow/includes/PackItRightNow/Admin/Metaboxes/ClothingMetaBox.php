...
class ClothingMetaBox {
	 ...
	 */
	function register() {
		add_action( 'add_meta_boxes', array( $this, 'packitrightnow_clothing_metaboxes' ) );
		add_action( 'save_post', array( $this, 'packitrightnow_clothing_save_data' ) );
	}
	 ...
	 * @return void
	 */
	function packitrightnow_clothing_metaboxes() {
		add_meta_box(
			'configuration',
			__( 'Configuration', 'packitrightnow_com' ),
			array( $this, 'packitrightnow_clothing_callback' ),
			CLOTHING_POST_TYPE
		);
	}
	 ...
	 * @return void
	 */
	function packitrightnow_clothing_callback( $post ) {
		# Add a nonce field so we can check for it later.
		wp_nonce_field( 'packitrightnow_clothing_save_data', 'packitrightnow_clothing_nonce' );

		# Call AccessoryFinder class methods.
		$finder = new ClothingFinder( $post->ID );
		...
		<table style="width: 100%;">
			<tr>
				<td>
					<label for="_featured"><?php echo esc_html( __( 'Feature Clothing', 'packitrightnow_com' ) ); ?></label>
				</td>
				<td>
	 ...
	 * @return void
	 */
	function packitrightnow_clothing_save_data( $post_id ) {
		/**
		 * We need to verify this came from our screen and with proper authorization,
		 * because the save_post action can be triggered at other times.
		 */
		if ( ! isset( $_POST['packitrightnow_clothing_nonce'] ) ) {
			return;
		}

		# Verify that the nonce is valid.
		if ( ! wp_verify_nonce( $_POST['packitrightnow_clothing_nonce'], 'packitrightnow_clothing_save_data' ) ) {
			return;
		}
